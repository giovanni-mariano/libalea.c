...
/**
 * @file mc_plotter_experimental.c
 * @brief CSG geometry slice plotter — EXPERIMENTAL raycast variant
 *
 * Identical to mc_plotter, except the default grid cell/material fill uses
 * alea_find_cells_grid_raycast() instead of the normal per-pixel point-query
 * fill. Use it to eyeball the scanline raycast 2D plot and diff it against
 * mc_plotter's reference output.
 *
 * The stricter ray-interval coverage path remains available with
 * --ray-intervals; it traces one ray per image row, classifies intervals by
 * exact midpoint coverage, and produces the normal none/one/multi coverage
 * arrays. Pass --reference to fall back to the point-query coverage fill for
 * an A/B comparison.
 *
...
/* Experimental: use scanline raycast fill by default; --reference switches
 * back to the point-query fill for A/B comparison. Ray-interval coverage stays
 * available as a stricter diagnostic mode via --ray-intervals. */
static int g_use_raycast = 1;
static int g_use_ray_intervals = 0;

static int plot_prep_stats_enabled(void) {
    return getenv("ALEA_PLOT_ERROR_STATS") != NULL ||
           getenv("ALEA_PLOT_PREP_STATS") != NULL;
}

static int prepare_plot_cache_step(alea_system_t* sys,
                                   unsigned flags,
                                   const char* label,
                                   int print_stats) {
    double t0 = get_time_ms();
    int rc = alea_system_prepare_query_caches(sys, flags);
    double ms = get_time_ms() - t0;
    if (print_stats) {
        printf("    [PLOT PREP] %-12s %8.1f ms  rss=%.1f MB\n",
               label, ms, get_rss_mb());
        fflush(stdout);
    }
    return rc;
...
static int prepare_plot_query_acceleration(alea_system_t* sys) {
    if (!g_use_raycast)
        return alea_prepare_query_acceleration(sys);

    int print_stats = plot_prep_stats_enabled();
    if (print_stats) {
        printf("\n");
        fflush(stdout);
    }

    if (prepare_plot_cache_step(sys, ALEA_CACHE_UNIVERSE,
                                "universe", print_stats) != 0)
        return -1;
    if (print_stats) {
        printf("    [PLOT PREP] model        cells=%zu surfaces=%zu has_lattice=%d\n",
               alea_vec_count(&sys->cells), alea_vec_count(&sys->surfaces),
               sys->has_lattice ? 1 : 0);
        fflush(stdout);
    }
    if (prepare_plot_cache_step(sys, ALEA_CACHE_SURFACE_BVH,
                                "surface_bvh", print_stats) != 0)
        return -1;

    if (prepare_plot_cache_step(sys, ALEA_CACHE_CELL_SURFACES,
                                "cell_surfs", print_stats) != 0)
        return -1;

    return prepare_plot_cache_step(sys, ALEA_CACHE_HIER_SPATIAL,
                                   "hier_spatial", print_stats);
}
...
static void print_ray_interval_stats(const char* label) {
    alea_ray_interval_stats_t rs = alea_ray_interval_stats_get();
    if (rs.rows == 0 && rs.rays == 0 && rs.fallback_invocations == 0)
        return;

    printf("    [PLOT ERROR STATS] %s ray intervals: rows=%zu rays=%zu ray_errors=%zu hits=%zu hits_max=%zu intervals=%zu exact_midpoints=%zu pixels=%zu fallbacks=%zu verify_samples=%zu verify_mismatch=%zu boundary_checked=%zu suppressed=%zu retained=%zu inconclusive=%zu\n",
           label, rs.rows, rs.rays, rs.ray_errors, rs.hits_total,
           rs.hits_max, rs.intervals, rs.interval_exact_queries,
           rs.pixels_filled, rs.fallback_invocations,
           rs.verifier_samples, rs.verifier_mismatches,
           rs.boundary_checked, rs.boundary_suppressed,
           rs.boundary_retained, rs.boundary_inconclusive);
}
...
static int render_plot(alea_system_t* sys, const plot_params_t* p, int verbose) {
    ...
    int verify_interval = (verify_env && atoi(verify_env) > 0)
        ? atoi(verify_env) : 0;
    /* Ray-interval coverage is its own experimental exact-like mode. Keep
     * plotting diagnostics enabled, but do not stack the other coverage
     * refiners on top of it. */
    if (g_use_ray_intervals) {
        selective_exact_coverage = 0;
        full_exact_coverage = 0;
        tile_exact_coverage = 0;
        path_exact_coverage = 0;
    }
    if (g_use_ray_intervals || plot_error_stats) {
    ...
    unsigned coverage_flags = g_use_ray_intervals
        ? ALEA_GRID_COVERAGE_RAY_INTERVALS
        : (full_exact_coverage ? ALEA_GRID_COVERAGE_EXACT
                               : ALEA_GRID_COVERAGE_FAST);
    if (secondary_ids) coverage_flags |= ALEA_GRID_SECONDARY_CELL_IDS;
    if (path_exact_coverage && !g_use_ray_intervals)
        coverage_flags |= ALEA_GRID_PATH_IDS;
    int rc;
    if (g_use_ray_intervals) {
    ...
    } else if (g_use_raycast) {
        rc = alea_find_cells_grid_raycast(
                 sys, &view, p->width, p->height, -1,
                 cell_ids, material_ids, errors);
        if (rc == 0) {
            for (int i = 0; i < num_pixels; i++) {
                if (coverage) {
                    coverage[i] = (errors[i] == GRID_ERR_OVERLAP)
                        ? ALEA_COVERAGE_MULTI
                        : (errors[i] == GRID_ERR_UNDEFINED || cell_ids[i] < 0)
                            ? ALEA_COVERAGE_NONE
                            : ALEA_COVERAGE_ONE;
                }
                if (secondary_ids) secondary_ids[i] = -1;
            }
        }
    } else {
    ...
    if (verbose) {
        ...
        if (g_use_ray_intervals) {
            printf("    Fill: experimental ray-interval coverage (ALEA_GRID_COVERAGE_RAY_INTERVALS)\n");
        } else if (g_use_raycast) {
            printf("    Fill: scanline raycast segments (alea_find_cells_grid_raycast)\n");
        } else if (full_exact_coverage) {
    ...
    if (g_use_ray_intervals && (verbose || plot_error_stats)) {
        print_ray_interval_stats("grid");
        print_point_coverage_stats("ray interval midpoints");
    }
        ...
        if (err_curves) {
            ...
            if (selective_exact_coverage && !full_exact_coverage &&
                ...
                new_overlaps = alea_check_grid_overlaps_curves_coverage(
                    ...
                    coverage, errors);
            } else if (!g_use_ray_intervals &&
                       !full_exact_coverage && !tile_exact_coverage &&
                       !path_exact_coverage) {
...
static void parse_options(const char* line, plot_params_t* p) {
    ...
    /* Experimental fill selection (global, applies to subsequent plots). */
    if (strstr(line, "reference")) {
        g_use_raycast = 0;
        g_use_ray_intervals = 0;
    } else if (strstr(line, "ray-intervals")) {
        g_use_raycast = 1;
        g_use_ray_intervals = 1;
    } else if (strstr(line, "raycast")) {
        g_use_raycast = 1;
        g_use_ray_intervals = 0;
    }
...
static int run_batch(const char* batch_file,
    ...
    if (prepare_plot_query_acceleration(sys) != 0) {
        printf("FAILED: %s\n", alea_error());
        destroy_model(&model);
        fclose(f);
        return 1;
...
static void print_usage(const char* prog) {
    ...
    fprintf(stderr, "  --raycast               Use scanline raycast fill (default)\n");
    fprintf(stderr, "  --ray-intervals         Use exact ray-interval coverage diagnostics\n");
    fprintf(stderr, "  --reference             Use point-query coverage fill (mc_plotter behaviour)\n");
...
int main(int argc, char** argv) {
    ...
    for (int i = 9; i < argc; i++) {
        if (strncmp(argv[i], "--labels=", 9) == 0) {
            ...
            debug_curves = 1;
        } else if (strcmp(argv[i], "--raycast") == 0) {
            g_use_raycast = 1;
            g_use_ray_intervals = 0;
        } else if (strcmp(argv[i], "--ray-intervals") == 0) {
            g_use_raycast = 1;
            g_use_ray_intervals = 1;
