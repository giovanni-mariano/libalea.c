...
static bool puts_raw(alea_xml_writer_t* x, const char* s, size_t n);
...
static inline bool puts_cstr(alea_xml_writer_t* x, const char* s)
{
...
static bool putc_raw(alea_xml_writer_t* x, char c)
{
...
static bool newline_and_indent(alea_xml_writer_t* x)
{
  if (!putc_raw(x, '\n')) return false;
...
static bool maybe_wrap_for_attr(alea_xml_writer_t* x, size_t incoming_len)
{
  if (!x->pretty) return true;
...
static bool put_escaped(alea_xml_writer_t* x, const char* s, bool for_attr)
{
  for (; *s; ++s) {
    switch (*s) {
...
static bool put_text_wrapped(alea_xml_writer_t* x, const char* s)
{
...
{
  alea_xml_writer_init_ex(x, arena, initial_capacity, 100, 2, true);
}
...
bool alea_xml_writer_attribute_i(alea_xml_writer_t* x, const char* name, int value)
{
  ...
  if (n <= 0) return false;
  return alea_xml_writer_attribute(x, name, buf);
}
