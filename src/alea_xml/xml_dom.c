...
static bool decode_entity(xml_lexer_t* lex) {
    ...
    if (strcmp(buf, "lt") == 0) token_append(lex, '<');
        ...
        long code;
        if (buf[1] == 'x') {
            code = strtol(buf + 2, &end, 16);
        } else {
            code = strtol(buf + 1, &end, 10);
        }
        if (!end || *end || code <= 0 || code > 0x10ffff ||
            (code >= 0xd800 && code <= 0xdfff)) {
            set_error(lex, "Invalid numeric character reference");
            return false;
        }
        if (code < 0x80) {
            token_append(lex, (char)code);
        } else if (code < 0x800) {
            token_append(lex, (char)(0xc0 | (code >> 6)));
            token_append(lex, (char)(0x80 | (code & 0x3f)));
        } else if (code < 0x10000) {
            token_append(lex, (char)(0xe0 | (code >> 12)));
            token_append(lex, (char)(0x80 | ((code >> 6) & 0x3f)));
            token_append(lex, (char)(0x80 | (code & 0x3f)));
        } else {
            token_append(lex, (char)(0xf0 | (code >> 18)));
            token_append(lex, (char)(0x80 | ((code >> 12) & 0x3f)));
            token_append(lex, (char)(0x80 | ((code >> 6) & 0x3f)));
            token_append(lex, (char)(0x80 | (code & 0x3f)));
        }
    } else {
        set_error(lex, "Unknown entity reference");
        return false;
    }
...
static void next_token_content(xml_lexer_t* lex) {
    ...
    if (c == '\0') {
        ...
        if (c == '/') {
            ...
            /* Comments are supported; DTD/entity declarations are forbidden. */
            if (peek_n(lex, 1) == '-' && peek_n(lex, 2) == '-') {
                advance(lex); advance(lex); advance(lex);
                skip_comment(lex);
            } else {
                set_error(lex, "DTD and entity declarations are not allowed");
            }
        } else if (c == '?') {
            advance(lex);
            skip_pi(lex);
...
typedef struct {
    xml_lexer_t lex;
    alea_xml_dom_doc_t* doc;
    alea_xml_dom_element_t* current;
    bool in_tag;
} xml_parser_t;

static alea_xml_dom_element_t* new_element(xml_parser_t* p, const char* tag_name) {
    alea_xml_dom_element_t* elem = (alea_xml_dom_element_t*)arena_alloc(
        &p->doc->arena, sizeof(alea_xml_dom_element_t));
    if (!elem) return NULL;
...
static bool add_attr(xml_parser_t* p, alea_xml_dom_element_t* elem,
                     const char* name, const char* value) {
    if (elem->attr_count >= elem->attr_capacity) {
        size_t new_cap = elem->attr_capacity ? elem->attr_capacity * 2 : 4;
        alea_xml_dom_attr_t* new_attrs = (alea_xml_dom_attr_t*)arena_alloc(
            &p->doc->arena, new_cap * sizeof(alea_xml_dom_attr_t));
        if (!new_attrs) return false;

        if (elem->attrs && elem->attr_count > 0) {
            memcpy(new_attrs, elem->attrs, elem->attr_count * sizeof(alea_xml_dom_attr_t));
        }
        elem->attrs = new_attrs;
        elem->attr_capacity = new_cap;
    }

    alea_xml_dom_attr_t* attr = &elem->attrs[elem->attr_count++];
...
static bool add_child(xml_parser_t* p, alea_xml_dom_element_t* parent,
                      alea_xml_dom_element_t* child) {
    if (parent->child_count >= parent->child_capacity) {
        size_t new_cap = parent->child_capacity ? parent->child_capacity * 2 : 4;
        alea_xml_dom_element_t** new_children = (alea_xml_dom_element_t**)arena_alloc(
            &p->doc->arena, new_cap * sizeof(alea_xml_dom_element_t*));
        if (!new_children) return false;

        if (parent->children && parent->child_count > 0) {
            memcpy(new_children, parent->children,
                   parent->child_count * sizeof(alea_xml_dom_element_t*));
        }
...
static bool parse_content(xml_parser_t* p, alea_xml_dom_element_t* parent) {
    while (true) {
...
static bool parse_element(xml_parser_t* p) {
    ...
    alea_xml_dom_element_t* elem = new_element(p, p->lex.token_sb.buf);
    if (!elem) {
...
alea_xml_dom_doc_t* alea_xml_dom_parse_string(const char* xml_string, size_t length) {
    ...
    alea_xml_dom_doc_t* doc = (alea_xml_dom_doc_t*)calloc(1, sizeof(alea_xml_dom_doc_t));
    if (!doc) return NULL;
...
alea_xml_dom_doc_t* alea_xml_dom_parse_file(const char* filename) {
    ...
    alea_xml_dom_doc_t* doc = alea_xml_dom_parse_string(content, read);
    free(content);
...
int alea_xml_dom_get_attr_int(const alea_xml_dom_element_t* elem,
                             const char* attr_name,
                             int default_value) {
    const char* val = alea_xml_dom_get_attr(elem, attr_name);
