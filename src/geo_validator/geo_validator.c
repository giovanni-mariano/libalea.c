...
int alea_validate_geometry(alea_system_t* sys,
    ...
    int workers = 1;
#ifdef _OPENMP
    if (!omp_in_parallel()) workers = omp_get_max_threads();
#endif
    if ((size_t)workers > ray_count) workers = (int)ray_count;
    ...
    for (size_t ray_index = 0; ray_index < ray_count; ray_index++) {
        generate_validation_ray(&rng, &bounds, &rays[ray_index]);
        alea_geom_validator_result_init(&ray_results[ray_index]);
    }

    /* Workers retain every per-ray finding. The caller applies global limits
     * in ray order so OpenMP scheduling cannot change the public receipt. */
...
#endif
    for (int ray_index = 0; ray_index < (int)ray_count; ray_index++) {
        if (alea_interrupted_internal()) {
            statuses[ray_index] = -1;
            continue;
        }
        statuses[ray_index] = validate_one_ray(
            sys, &rays[ray_index], t_max, &worker_options,
            &ray_results[ray_index]);
    }
    ...
    for (size_t ray_index = 0;
         ray_index < ray_count && !result->truncated; ray_index++) {
        if (statuses[ray_index] != 0) {
            merge_failed = 1;
            break;
        }
