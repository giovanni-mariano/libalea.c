...
ALL_OBJS = $(CORE_LIB_OBJS) $(NUCDATA_OBJS) $(TRANSPORT_OBJS) $(MCNP_MODULE_OBJS) $(OPENMC_MODULE_OBJS) $(SERPENT_MODULE_OBJS)
...
modules: structure $(LIB_MCNP) $(LIB_OPENMC) $(LIB_SERPENT) $(LIB_NUCDATA) $(LIB_TRANSPORT)
...
ifeq ($(TEST_UNAME_S),Darwin)
  TEST_LIBS = -Wl,-force_load,$(LIB_MCNP) -Wl,-force_load,$(LIB_OPENMC) -Wl,-force_load,$(LIB_SERPENT) -Wl,-force_load,$(LIB_TRANSPORT) -Wl,-force_load,$(LIB_NUCDATA) $(LIB_CORE)
else
  TEST_LIBS = -Wl,--whole-archive $(LIB_MCNP) $(LIB_OPENMC) $(LIB_SERPENT) $(LIB_TRANSPORT) $(LIB_NUCDATA) -Wl,--no-whole-archive $(LIB_CORE)
endif

# Unit tests
$(BIN_DIR)/tests/unit/%$(EXEEXT): $(UNIT_TEST_DIR)/%.c $(LIB_CORE) $(LIB_MCNP) $(LIB_OPENMC) $(LIB_SERPENT) $(LIB_TRANSPORT) $(LIB_NUCDATA) | $(BIN_DIR)/tests/unit
	@echo "LD  $@"
...
# Integration tests
$(BIN_DIR)/tests/integration/%$(EXEEXT): $(INTEGRATION_TEST_DIR)/%.c $(LIB_CORE) $(LIB_MCNP) $(LIB_OPENMC) $(LIB_SERPENT) $(LIB_TRANSPORT) $(LIB_NUCDATA) | $(BIN_DIR)/tests/integration
	@echo "LD  $@"
	@$(CC) $(TEST_CFLAGS) $(TEST_INCLUDES) $< $(TEST_LIBS) $(LDFLAGS) -o $@
