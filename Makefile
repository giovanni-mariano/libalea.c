...
# Core library sources (no format or visualization modules)
CORE_LIB_SRCS = \
	$(CORE_SRCS) \
	$(UTIL_SRCS) \
	$(PRIMITIVES_SRCS)
...
MODULE_SLICE_SRCS = $(SLICE_SRCS)
MODULE_RENDER_SRCS = $(RENDER_SRCS)
MODULE_MESH_SRCS = $(MESH_SRCS)
...
# All library sources (core + all modules, for backwards compatibility)
LIB_SRCS = $(CORE_LIB_SRCS) $(MODULE_RAYCAST_SRCS) $(MODULE_SLICE_SRCS)
...
# Core library objects (no format or visualization modules)
...
MODULE_SLICE_OBJS = $(SLICE_OBJS)
MODULE_RENDER_OBJS = $(RENDER_OBJS)
MODULE_MESH_OBJS = $(MESH_OBJS)

# All objects (for full library)
ALL_OBJS = $(CORE_LIB_OBJS) $(MCNP_MODULE_OBJS) $(OPENMC_MODULE_OBJS) $(MODULE_RAYCAST_OBJS) $(MODULE_SLICE_OBJS) $(MODULE_RENDER_OBJS) $(MODULE_MESH_OBJS)
...
# Core library (parsing, evaluation, export - no visualization)
...
LIB_RAYCAST = $(BIN_DIR)/libalea_raycast.a
LIB_SLICE = $(BIN_DIR)/libalea_slice.a
LIB_RENDER = $(BIN_DIR)/libalea_render.a
LIB_MESH = $(BIN_DIR)/libalea_mesh.a
...
modules: structure $(LIB_MCNP) $(LIB_OPENMC) $(LIB_RAYCAST) $(LIB_SLICE) $(LIB_RENDER) $(LIB_MESH)

# Build everything (core + modules)
lib: lib-core modules $(LIB)
...
structure:
	...
	@mkdir -p $(BUILD_DIR)/mesh
	@mkdir -p $(BUILD_DIR)/lua
	@mkdir -p $(BUILD_DIR)/lua_bind
	@mkdir -p $(BUILD_DIR)/linenoise
	@mkdir -p $(BIN_DIR)
	@mkdir -p $(BIN_DIR)/tests/unit
	@mkdir -p $(BIN_DIR)/tests/integration
...
# Raycast module (optional)
$(LIB_RAYCAST): $(MODULE_RAYCAST_OBJS) | $(BIN_DIR)
	@echo "AR  $@"
	@ar rcs $@ $^
	@echo "✓ Built raycast module: $@"

# Slice module (optional)
$(LIB_SLICE): $(MODULE_SLICE_OBJS) | $(BIN_DIR)
	@echo "AR  $@"
	@ar rcs $@ $^
	@echo "✓ Built slice module: $@"

# Render module (optional)
$(LIB_RENDER): $(MODULE_RENDER_OBJS) | $(BIN_DIR)
	@echo "AR  $@"
	@ar rcs $@ $^
	@echo "✓ Built render module: $@"

# Mesh export module (optional)
$(LIB_MESH): $(MODULE_MESH_OBJS) | $(BIN_DIR)
	@echo "AR  $@"
	@ar rcs $@ $^
	@echo "✓ Built mesh module: $@"

# Full library (everything combined, backwards compatible)
...
# Platform-specific whole-archive flags for CLI (same as tests)
CLI_UNAME_S := $(shell uname -s)
ifeq ($(CLI_UNAME_S),Darwin)
  CLI_LIBS = $(LIB_MESH) $(LIB_RENDER) $(LIB_SLICE) -Wl,-force_load,$(LIB_RAYCAST) -Wl,-force_load,$(LIB_MCNP) -Wl,-force_load,$(LIB_OPENMC) $(LIB_CORE)
else
  CLI_LIBS = $(LIB_MESH) $(LIB_RENDER) $(LIB_SLICE) -Wl,--whole-archive $(LIB_RAYCAST) $(LIB_MCNP) $(LIB_OPENMC) -Wl,--no-whole-archive $(LIB_CORE)
endif

$(ALEA_CLI): $(LUA_BIND_DIR)/lua_main.c $(LUA_BIND_OBJS) $(LINENOISE_OBJ) $(LUA_OBJS) $(LIB_CORE) $(LIB_MCNP) $(LIB_OPENMC) $(LIB_RAYCAST) $(LIB_SLICE) $(LIB_RENDER) $(LIB_MESH) | $(BIN_DIR)
	@echo "LD  $@"
...
# Format modules override weak stubs in core, need to force all symbols to be included
TEST_UNAME_S := $(shell uname -s)
ifeq ($(TEST_UNAME_S),Darwin)
  # macOS: use -force_load for each archive that needs whole-archive behavior
  TEST_LIBS = $(LIB_MESH) $(LIB_RENDER) $(LIB_SLICE) -Wl,-force_load,$(LIB_RAYCAST) -Wl,-force_load,$(LIB_MCNP) -Wl,-force_load,$(LIB_OPENMC) $(LIB_CORE)
else
  # Linux/Windows: use --whole-archive / --no-whole-archive
  TEST_LIBS = $(LIB_MESH) $(LIB_RENDER) $(LIB_SLICE) -Wl,--whole-archive $(LIB_RAYCAST) $(LIB_MCNP) $(LIB_OPENMC) -Wl,--no-whole-archive $(LIB_CORE)
endif

# Unit tests - link against modules and core
$(BIN_DIR)/tests/unit/%: $(UNIT_TEST_DIR)/%.c $(LIB_CORE) $(LIB_MCNP) $(LIB_OPENMC) $(LIB_SLICE) $(LIB_RAYCAST) $(LIB_RENDER) $(LIB_MESH) | $(BIN_DIR)/tests/unit
	@echo "LD  $@"
	@$(CC) $(TEST_CFLAGS) $(TEST_INCLUDES) $< $(TEST_LIBS) $(LDFLAGS) -o $@

# Integration tests
$(BIN_DIR)/tests/integration/%: $(INTEGRATION_TEST_DIR)/%.c $(LIB_CORE) $(LIB_MCNP) $(LIB_OPENMC) $(LIB_SLICE) $(LIB_RAYCAST) $(LIB_RENDER) $(LIB_MESH) | $(BIN_DIR)/tests/integration
	@echo "LD  $@"
	@$(CC) $(TEST_CFLAGS) $(TEST_INCLUDES) $< $(TEST_LIBS) $(LDFLAGS) -o $@
...
help:
	...
	@echo "  src/             - Source code (organized by layer)"
	@echo "    core/          - Core CSG system (NEW)"
	@echo "    util/          - Memory management (arena)"
...
.PHONY: all lib tests test test-unit test-integration test-valgrind clean distclean tree help structure
