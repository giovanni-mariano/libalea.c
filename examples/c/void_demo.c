...
int main(void) {
    ...
    /* Tune octree for fast demo (default depth=8 min_size=0.1 is thorough but slow) */
    ...
    for (size_t i = 0; i < display_limit; i++) {
        ...
        printf("  [%2zu] [%6.2f,%6.2f] x [%6.2f,%6.2f] x [%6.2f,%6.2f]"
               ...
               box.min_z, box.max_z, dx, dy, dz, vol);
    }
    ...
    for (size_t i = display_limit; i < count; i++) {
        alea_bbox_t box;
        alea_void_get(vr, i, &box);
        total_void_vol += (box.max_x - box.min_x) *
                          (box.max_y - box.min_y) *
                          (box.max_z - box.min_z);
    }
