...
/**
 * Generate a type-safe result type.
 *
 * Usage:
 *   ALEA_DEFINE_RESULT(alea_node, alea_node_id_t);
 *   // Creates: alea_node_result_t with { .error, .value }
 */
#define ALEA_DEFINE_RESULT(name, type)   \
    typedef struct {                     \
        alea_error_t error;               \
        type value;                      \
    } name##_result_t

/** Result for void operations that can fail */
typedef struct {
    alea_error_t error;
} alea_result_t;

/** Result for pointer returns (system creation, etc.) */
ALEA_DEFINE_RESULT(alea_ptr, void*);

/** Result for system pointer returns */
ALEA_DEFINE_RESULT(alea_system, alea_system_t*);

/** Result for node ID returns */
ALEA_DEFINE_RESULT(alea_node, alea_node_id_t);

/** Result for primitive ID returns */
ALEA_DEFINE_RESULT(alea_primitive, alea_primitive_id_t);

/** Result for integer returns (indices, small counts) */
ALEA_DEFINE_RESULT(alea_int, int);

/** Result for size_t returns (counts, sizes) */
ALEA_DEFINE_RESULT(alea_size, size_t);

/** Result for boolean returns */
ALEA_DEFINE_RESULT(alea_bool, bool);

/** Result for double returns (distances, evaluations) */
ALEA_DEFINE_RESULT(alea_double, double);

/* --- Result Constructors --- */

/** Create success result with value: { .error = ALEA_OK, .value = val } */
#define ALEA_OK_VAL(val) { .error = ALEA_OK, .value = (val) }

/** Create success result without value (for alea_result_t) */
#define ALEA_SUCCESS { .error = ALEA_OK }

/** Create failure result: { .error = err } */
#define ALEA_FAIL(err) { .error = (err) }

/* --- Result Checking --- */

/** Check if result is success */
#define ALEA_IS_OK(res) ((res).error == ALEA_OK)

/** Check if result is error */
#define ALEA_IS_ERR(res) ((res).error != ALEA_OK)
